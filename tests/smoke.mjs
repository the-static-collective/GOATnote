...
assert.equal($('returnMargin').disabled,false,'historical source enables optional return');
$('returnMargin').click();
$('marginText').value = 'a return witness from the later saved context';
$('saveMargin').click();
note = db().notes[0];
const returned = note.margins.find(m => m.returnLink);
assert(returned,'return action created a source-bound margin');
assert.equal(returned.versionId,versionId,'return margin stays attached to historic source');
assert.equal(returned.returnLink.sourceVersionId,versionId);
assert(note.versions.some(v=>v.id===returned.returnLink.returnVersionId),'later return context exists');
assert.equal(note.versions.find(v=>v.id===versionId).text,source,'return leaves original immutable');
...
console.log('GOATnote smoke: source/version, margin and return checks passed. Browser/storage testing still required.');
