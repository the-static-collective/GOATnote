...
$('attentionExport').click();
const exportedNote=JSON.parse($('attentionHandoff').value);
assert.equal(exportedNote.schema,'attention-crossing.handoff/v0.1');
assert.equal(exportedNote.source_app,'goatnote');
assert.equal(exportedNote.source_record_id,note.attention.at(-1).id);
assert.equal(exportedNote.source_target_id,note.attention.at(-1).key);
assert.deepEqual(exportedNote.dimensions,['joyful']);
assert.equal(exportedNote.evidence,'source-export/self-reported');
...
$('attentionExport').click();
const exportedPassage=JSON.parse($('attentionHandoff').value);
assert.equal(exportedPassage.source_record_id,crossing.id);
assert.match(exportedPassage.source_locator,/\/version\/.+\/characters\/2-12$/);
assert.equal(exportedPassage.label,crossing.anchor.quote);
